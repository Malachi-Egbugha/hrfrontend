...
const Lineleaveapply = () => {
  ...
  const submitstatus = async (i, Id) => {
    ...
    updatedleave.error ? setError(updatedleave.error) : loadDsiplayLeaves();
  };
  const showError = () => (
    <div
            ...
            <tbody>
              {displayLeaves.map((u, i) => (
                ...
                >
                  <td>{1 + i}</td>
                  <td>
                    {u.lineleavestatus !== "pending" ? (
                      <button
                        onClick={() => submitstatus(i, u._id)}
                        type="button"
                  ...
                  <td
                    style={{
                      color: "#fff",
                      backgroundColor:
                        u.lineleavestatus === "pending" ? "#D55451" : "#2A1372",
                    }}
                  >
                    {u.lineleavestatus}
                  </td>
                  ...
                  <td>{u.department}</td>
                  <td>{u.leavetype}</td>
                  <td>{format(parseISO(u.leavestart), "MM/dd/yyyy")}</td>
                  <td>{format(parseISO(u.leaveend), "MM/dd/yyyy")}</td>
                  <td>{u.comment}</td>
                </tr>
              ))}
            </tbody>
